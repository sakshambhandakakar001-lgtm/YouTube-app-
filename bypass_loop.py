...
# Remove old login/logout/signup definitions
patterns = [
...
# Bypass any forced redirects inside original home route
code = code.replace("return redirect('/login')", "# return redirect('/login')")
code = code.replace("return redirect(url_for('login'))", "# return redirect(url_for('login'))")

# Add updated auth system
auth_system = """
...
if "if __name__ ==" in code:
    ...
    final_code = parts[0] + auth_system + "\nif __name__ ==" + parts[1]
else:
    final_code = code + auth_system

with open('app.py', 'w') as f:
    f.write(final_code)

print("✅ Original App Restored & Redirect Loop Bypassed!")
